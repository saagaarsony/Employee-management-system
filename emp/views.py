...
def profile(request):
    ...
    if request.method == "POST":
        ...
        employee.empcode = ec
        employee.empdep = dept
        employee.designation = designation
        employee.sereport = sr
...
def edit_edu(request,pid):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            error = "Yes"
    return render(request, 'edit_edu.html', locals())


def edit_exp(request,pid):
    if not request.user.is_authenticated:
        return redirect('emp_login')
    error = ""
    user = User.objects.get(id=pid)
